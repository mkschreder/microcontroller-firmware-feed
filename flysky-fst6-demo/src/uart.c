...
static  struct uart_device _devices[] = {
	{
		.dev = USART1, 
		.gpio = GPIOA, 
...
static struct cbuf uart0_rx_buf; 
static struct cbuf uart1_rx_buf; 
static struct cbuf uart2_rx_buf; 
static struct cbuf uart3_rx_buf; 
static struct cbuf uart4_rx_buf; 
static struct cbuf uart5_rx_buf; 
...
int8_t uart_init(uint8_t dev_id, uint32_t baud){
	...
	switch(dev_id){
		case 0: 
			cbuf_init(&uart0_rx_buf, conf->rx_buf, UART_RX_BUFFER_SIZE); 
			break; 
	}
...
uint16_t uart_waiting(uint8_t dev_id){
	...
	USART_ITConfig(dev, USART_IT_RXNE, DISABLE);
	switch(dev_id){
		case 0: cn = cbuf_get_waiting(&uart0_rx_buf); break; 
		case 1: cn = cbuf_get_waiting(&uart1_rx_buf); break; 
		case 2: cn = cbuf_get_waiting(&uart2_rx_buf); break; 
		case 3: cn = cbuf_get_waiting(&uart3_rx_buf); break; 
		case 4: cn = cbuf_get_waiting(&uart4_rx_buf); break; 
		case 5: cn = cbuf_get_waiting(&uart5_rx_buf); break; 
	}
	USART_ITConfig(dev, USART_IT_RXNE, ENABLE);
	return cn; 
}
